...
Based on the development that has been conducted, the conclusions that can be drawn are as follows:

1. The implementation of fine-tuning on the pre-trained EfficientNetV2B2 model has proven to be superior in terms of predictive performance and training efficiency compared to custom CNN and VGG-19 architectures for the Rupiah banknote authenticity identification system. EfficientNetV2B2 achieved the highest scores across various evaluation metrics, including accuracy (0.94), F1-Macro (0.93), Precision-Macro (0.92), Recall-Macro (0.93), and ROC-AUC Score (0.98), demonstrating exceptional ability in detecting counterfeit money and avoiding false positives. Additionally, this model exhibited better training time efficiency, with an average epoch count of 43.88 and a balanced total training time of 1847.50 seconds, making it the best choice, especially for supporting visually impaired individuals in identifying Rupiah banknote authenticity with high confidence.

2. The use of image augmentation and hyperparameter tuning with Optuna in this development did not significantly enhance the counterfeit detection capability of the EfficientNetV2B2 model. After applying image augmentation, performance declined in some metrics, including Recall-Macro, which dropped to 0.77, indicating an increase in false negatives. Although the ROC-AUC Score remained high, these results suggest that the applied image augmentation was not sufficiently effective in improving the model’s generalization. Similarly, hyperparameter tuning with Optuna also led to a decline in some metrics, such as Recall-Macro, which fell to 0.81. Although the tuning process successfully identified optimal parameters that minimized validation loss, the results were not adequate to enhance the overall model performance. Therefore, further exploration of more suitable augmentation transformations and longer hyperparameter tuning durations is required to achieve the expected performance improvements.

3. The characteristics of authentic Rupiah banknotes that can be identified using fine-tuning on a pre-trained model, particularly through Grad-CAM, include various security features that determine authenticity. Based on testing conducted on 2016 and 2022 emission banknotes, the EfficientNetV2B2 model was able to identify and give significant attention to features such as security thread, watermark, money color, color-shifting ink, and microtext on 2016 banknotes. These features were marked with higher intensity on the heatmap, indicating that the model predominantly used these features in the authentication process. Additionally, for 2022 emission banknotes, the model successfully recognized updated security features, such as a wider security thread and a hidden image containing the denomination and year of emission. Grad-CAM also highlighted the model's focus on microprint features and ink that changes color when viewed from different angles, demonstrating the model's ability to detect fine details that are crucial in verifying banknote authenticity. With this capability, the model not only recognizes more common security features but also other characteristics that provide additional layers of security, making it more effective in detecting banknote authenticity even under complex conditions.
